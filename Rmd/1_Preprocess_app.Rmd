...
- Missing values: na = c("XNA","NA","","NaN","?")
- convert charcter type into factor type: 
- remove duplicate column names and change the small letters
- add `_app` to all column names
...
app_train <- read_csv("~/Desktop/Home_Credit_Kaggle/csv/application_train.csv",na = c("XNA","NA","","NaN","?")) %>% 
  ...
  rename_all(. %>% tolower %>% str_c("_app",sep=""))
...
- Add `_add` to the added new column
...
app_train <- app_train %>% 
  mutate(DAYS_EMPLOYED = ifelse(DAYS_EMPLOYED == 365243, NA, DAYS_EMPLOYED),
         DAYS_LAST_PHONE_CHANGE = ifelse(DAYS_LAST_PHONE_CHANGE == 0, NA, DAYS_LAST_PHONE_CHANGE))
app_test <- app_test %>% 
  mutate(DAYS_EMPLOYED = ifelse(DAYS_EMPLOYED == 365243, NA, DAYS_EMPLOYED),
         DAYS_LAST_PHONE_CHANGE = ifelse(DAYS_LAST_PHONE_CHANGE == 0, NA, DAYS_LAST_PHONE_CHANGE))
```
...
```{r}
app_train <- app_train %>% 
  mutate(CREDIT_ANNUITY_PERC_add = AMT_CREDIT / AMT_ANNUITY,
         CREDIT_GOODS_PERC_add = AMT_CREDIT / AMT_GOODS_PRICE,
         CREDIT_INCOME_PERC_add = AMT_CREDIT / AMT_INCOME_TOTAL,
         INCOME_CHILDEREN_PERC_add = AMT_INCOME_TOTAL / (1 + CNT_CHILDREN),
         EMPLOYED_BIRTH_PERC_add = DAYS_EMPLOYED / DAYS_BIRTH,
         OWN_CAR_BIRTH_PERC_add = OWN_CAR_AGE / DAYS_BIRTH,
         CHILDREN_MEMBERS_PERC_add = CNT_CHILDREN / CNT_FAM_MEMBERS,
         OWN_CAR_EMPLOY_PERC_add = OWN_CAR_AGE / DAYS_EMPLOYED,
         INCOME_MEMBERS_PERC_add = AMT_INCOME_TOTAL / CNT_FAM_MEMBERS,
         ...
         CREDIT_MEMBERS_PERC_add = AMT_CREDIT / CNT_FAM_MEMBERS,
         CNT_NON_CHILD_add = CNT_FAM_MEMBERS - CNT_CHILDREN,
...
```{r eval=FALSE}
patterns <- c("EXT_TSNE_1", "EXT_TSNE_2")
# make new column
tmp1 <- app_train %>% 
...
```{r}
patterns <- c("AVG_TSNE_1", "AVG_TSNE_2")
# make new column
tmp1 <- app_train %>% 
  select_if(grepl("_AVG",names(.))) %>% 
  mutate_all(funs(ifelse(is.na(.),0,.))) %>% 
...
tmp <- c("HOUSETYPE_MODE","WALLSMATERIAL_MODE","FONDKAPREMONT_MODE","TOTALAREA_MODE","EMERGENCYSTATE_MODE")
# list
patterns <- c( "MODE_TSNE_1", "MODE_TSNE_2")
...
```{r}
# list
patterns <- c("MEDI_TSNE_1", "MEDI_TSNE_2")
