...
```{r eval=FALSE}
## for test
train_data <- train_data[1:100000,1:20]
test_data <- test_data[1:10000,1:20]
```
...
XgboostPred <- function(xgb_params,file_name,train_data,test_data){
  ...
  xgb_cv <- 
    xgb.cv(data = xgb.DMatrix(data = train_data %>% select(-TARGET) %>% select(alr$Feature) %>%  data.matrix(),
  ...
  # xgboost modeling  
  xgb_model <- 
    xgboost(data = xgb.DMatrix(data = train_data %>% select(-TARGET) %>% select(alr$Feature) %>% data.matrix(),
                               label = train_data$TARGET),
            params = xgb_params,
  ...
  xgb.importance(model = xgb_model) %>% 
    ...
    print()
  # predict
  pred_test <- predict(xgb_model,test_data %>% select(-SK_ID_CURR) %>% select(alr$Feature) %>% data.matrix())

  # make submit style
  submit <- data.frame(test_data,TARGET = pred_test) %>% 
    select(SK_ID_CURR,TARGET)
  # change format
  submit$SK_ID_CURR <- submit$SK_ID_CURR %>% as.integer()
...
# 番号要注意
XgboostSelect <- function(xgb_params,train_data,test_data){
  # not need column
  alr <- read_tsv("~/Desktop/Home_Credit_Kaggle/data/best_para.tsv") # for mac
  ...
  # xgboost cross validation to choice best parameter
  xgb_cv <- 
    xgb.cv(data = xgb.DMatrix(data = train_data %>% select(-TARGET) %>% select(-one_of(alr$Feature)) %>% data.matrix(),
                              label = train_data$TARGET),
           params = xgb_params,
           missing = NA,
           nfold = 5, 
  ...
  # xgboost modeling  
  xgb_model <- 
    xgboost(data = xgb.DMatrix(data = train_data %>% select(-TARGET) %>% select(-one_of(alr$Feature)) %>% data.matrix(),
                               label = train_data$TARGET),
            params = xgb_params,
            nrounds = xgb_cv$best_iteration, # max number of trees to build
            verbose = TRUE,                                         
            print_every_n = 10,
            early_stopping_rounds = 200 )
  tmp <- xgb.importance(model = xgb_model) %>% 
    select(Feature)
