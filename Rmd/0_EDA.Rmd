...
- Set up
    ...
    - import csv 
        ...
        - Add keyword to all column names for each data: (_app,_bureau,_pre,_pos,_ins,_cre)
    - export csv
...
bureau <- read_csv("~/Desktop/Home_Credit_Kaggle/input/csv/bureau.csv",na = c("XNA","NA","","NaN","?")) %>% 
  mutate_if(is.character, funs(factor(.))) %>% 
  clean_names() %>% # change small letters
  rename_all(. %>% tolower %>% str_c("_bureau",sep="")) 
